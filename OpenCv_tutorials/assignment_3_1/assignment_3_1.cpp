...
int main(int argc , char** argv)
{
   ...
   for (int i{0}; i<chessboard[1];i++)
   {
    ...
    std::string path = "/home/malika/Downloads/Calib/*.jpg";
    ...
    namedWindow("Corner_Image",WINDOW_KEEPRATIO);
    ...
    for(int i{0};i<images.size();i++)
    {
        ...
        if_sucess=findChessboardCorners(gray,cv::Size(chessboard[0],chessboard[1]),corner_points,CALIB_CB_ADAPTIVE_THRESH |CALIB_CB_FAST_CHECK | CALIB_CB_NORMALIZE_IMAGE);
       ...
       if(if_sucess)
       {
           ...
           if(i==0)
           {
            //    undis_test=imread(images[i]);
               undis_test=imread("/home/malika/Downloads/Calib/test.jpg");
        ...
        //    TermCriteria stop_critera(TERMCRIT_EPS| TERMCRIT_ITER,30,0.001);
        //    TermCriteria stop_critera(max_iter,30,0.001);
            
            //Perform the subpixel level corner detection
        //    cornerSubPix(gray,corner_points,cv::Size(11,11),cv::Size(-1,-1),stop_critera);
            cornerSubPix(gray,corner_points,cv::Size(11,11),cv::Size(-1,-1),TermCriteria(TermCriteria::MAX_ITER|TermCriteria::EPS, 30, 0.001));
    ...
    */
    ...
    namedWindow("Before and after undistorion",WINDOW_NORMAL);
    cv::resizeWindow("Before and after undistorion", 2400, 1200);
